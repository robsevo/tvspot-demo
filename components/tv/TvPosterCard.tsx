...
/** Smallest TMDB size that still looks right in a 176×264 tile. The ladders
 ...
 *  oversamples the 176×264 tile on a 1080p panel. */
 ...
 * driven by a remote instead of a thumb. 176×264 is picked so TWO rows still
 * fit the rails pane at 1080p; see the height budget in TvBrowseScreen.
 *
...
export default function TvPosterCard({
  ...
  return (
    <Link
      ...
      className={`block focus:outline-none ${fluid ? "w-full" : "w-[176px] shrink-0"}`}
    >
      ...
      <div
        className={`tv-card-shadow relative rounded-lg bg-[#1a242f] ring-1 ring-white/10 ${
          fluid ? "aspect-[2/3] overflow-hidden" : "h-[264px]"
        }`}
      >
        {art && !imgError ? (
          <img
      ...
      {sublabel && (
        <p className={`mt-2 text-base text-[#8197a4] truncate ${fluid ? "" : "w-[176px]"}`}>
          {sublabel}
        </p>
      )}
    </Link>
